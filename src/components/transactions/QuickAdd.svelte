...
<div class="fixed inset-0 bg-black/50 z-50 flex items-end sm:items-center justify-center" on:click|self={onClose}>
  <div class="bg-white w-full max-w-md rounded-t-3xl sm:rounded-2xl p-6 max-h-[90vh] overflow-y-auto">
    <!-- Header -->
    ...
    <div class="text-center mb-6">
      <div class="text-4xl font-bold text-gray-900 mb-2">{displayAmount}</div>
      <input
        type="date"
        bind:value={date}
        class="text-sm text-gray-500 bg-transparent border-none text-center"
      />
    </div>
    ...
    <div class="grid grid-cols-4 gap-2 mb-6">
      {#each ['7', '8', '9', 'C'] as key}
        <button
          ...
          class="h-12 rounded-lg text-xl font-medium {key === 'C' ? 'bg-red-100 text-red-600' : 'bg-gray-100 text-gray-900'} hover:bg-gray-200 transition-colors"
        >
          {key}
        </button>
      {/each}
      {#each ['4', '5', '6', '←'] as key}
        <button
          ...
          class="h-12 rounded-lg text-xl font-medium {key === '←' ? 'bg-gray-200' : 'bg-gray-100'} text-gray-900 hover:bg-gray-200 transition-colors"
        >
          {key}
        </button>
      {/each}
      {#each ['1', '2', '3', '.'] as key}
        <button
          ...
          class="h-12 rounded-lg text-xl font-medium bg-gray-100 text-gray-900 hover:bg-gray-200 transition-colors"
        >
          {key}
        </button>
      {/each}
      <button
        ...
        class="col-span-2 h-12 rounded-lg text-xl font-medium bg-gray-100 text-gray-900 hover:bg-gray-200 transition-colors"
      >
        0
      </button>
      <button
